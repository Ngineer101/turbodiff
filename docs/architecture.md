...
GitHub `workflow_run` events record checks against an immutable head in `change_checks`. Duplicate events upsert one workflow check; older timestamps cannot overwrite newer results. Events for other heads cannot change the current revision's readiness. Version 2 delivery publication hands factory-owned changes to `change_delivery@1`. Existing version 1 implementation runs remain executable. The change-scoped child uses the existing factory/stage/agent rows, immutable revisions and R2 artifacts; no second orchestration model or mutable agent payload is introduced.

The coordinator serializes scheduling on the change row and emits one queued stage at a time. Review, acceptance verification, repair and merge stages carry an immutable revision id in lifecycle events. All delivery gates are recomputed for a new head. Verification is a pure named agent with a typed per-criterion artifact; the stage independently runs the configured repository check and persists combined evidence. Missing criteria or unverified acceptance evidence require human action.

`assisted_delivery` repairs failing CI, blocking review, or acceptance checks and stops ready for a human merge. `full_delivery` also merges GitHub changes after fresh provider checks and sends the expected head SHA to GitHub's merge API. Only factory/automation changes linked to a delivery on a factory branch are eligible. Drafts, foreign heads, unknown/pending CI, incomplete review coverage, and non-mergeable PRs cannot auto-merge. Check-free GitHub heads have a 60-second discovery grace period. GitHub branch protection remains authoritative.

Repairs use the existing implementer repair operation, with immutable CI logs/review/verification evidence, a fresh checkout, credential-free remotes, and an ordinary non-force push after rechecking policy and head. There are at most three automatic repair attempts per change. A no-change repair, incomplete evidence, or exhausted budget produces a visible handoff; explicit delivery resumption rechecks evidence without resetting the repair budget.

PR and workflow webhooks wake the coordinator. The scheduled factory recovery also reconciles open eligible changes and re-enqueues durable queued stages, covering missed webhooks and queue publication failures. GitHub App installations need Actions read, Checks read and Commit statuses read permissions in addition to existing Contents/Pull requests write; subscribe to `workflow_run` for prompt CI reactions. Recovery provides polling fallback. No existing repository is silently opted into repair or merge: settings absent from the rewritten database resolve to `automatic_review`, and full delivery must be explicitly saved.
