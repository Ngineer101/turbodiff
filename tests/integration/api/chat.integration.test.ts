...
function send(
  ...
  return Effect.runPromise(
    Effect.gen(function* () {
      return yield* (yield* DeliveryService).createChatTurn(tenant.user, deliveryId, body);
...
describe('delivery chat execution', () => {
  it(
    'captures long-lived changes whose merge base lies beyond both shallow windows',
    () =>
      rollbackAfter(async () => {
        ...
        try {
          const local = localRuntime(root, true, async () => {
            expect(
              execFileSync(
                ...
                [
                  '-C',
                  join(root, `chat-${messages[0].stageRunId}`),
                  'rev-parse',
          ...
          for (const branch of ['feature', 'main']) {
            local.git('checkout', '-q', branch);
            for (let index = 0; index < 55; index++)
              local.git('commit', '--allow-empty', '-qm', `${branch}-${index}`);
          }
          const resolveRemote = local.runtime.remote;
          local.runtime.remote = async (...args) => {
            const remote = await resolveRemote(...args);
            return { ...remote, env: { GIT_REMOTE: `file://${join(root, 'origin')}` } };
          };
          ...
          expect((await latestChangeRevision(change.id))?.base_sha).toBe(
  ...
  it('saves diff comments and acceptance feedback for members without starting an agent', () =>
    rollbackAfter(async () => {
      const tenant = await createTenant('member');
      const { delivery, change } = await fixture(tenant);
      await updateChangeStatus(change.id, 'closed');
      const messages: RunFactoryMessage[] = [];
      const feedback = [
        'src/index.ts:12 (right)\nPlease rename this',
        'Acceptance contract feedback:\nKeep the existing behavior',
      ];
      for (const body of feedback) {
        const saved = await Effect.runPromise(
          Effect.gen(function* () {
            return yield* (yield* DeliveryService).createMessage(tenant.user, delivery.id, body);
          }).pipe(Effect.provide(service(messages))),
        );
        expect(saved).toMatchObject({ body, factoryRunId: null, status: null });
      }
      expect((await listDeliveryMessages(delivery.id)).map((entry) => entry.body)).toEqual(
        feedback,
      );
      expect(messages).toEqual([]);
      expect(
        await queryOne<{ count: number }>(
          sql`SELECT count(*)::int AS count FROM app.factory_runs WHERE delivery_id = ${delivery.id}`,
        ),
      ).toEqual({ count: 0 });
  ...
  it('does not publish files generated by a successful check', () =>
    rollbackAfter(async () => {
      const tenant = await createTenant();
      const { delivery } = await fixture(tenant);
      await execute(
        sql`UPDATE app.repositories SET settings = '{"checkCommand":"echo generated > generated.txt; echo formatted > fix.txt"}'::jsonb WHERE id = ${tenant.repositoryId}`,
      );
      const messages: RunFactoryMessage[] = [];
      await send(tenant, delivery.id, messages);
      const root = mkdtempSync(join(tmpdir(), 'turbodiff-chat-'));
      try {
        ...
        await executeFactoryStage(messages[0], local.runtime);
        expect(local.git('show', 'feature:fix.txt')).toBe('fixed');
        expect(local.git('ls-tree', '--name-only', 'feature')).not.toContain('generated.txt');
        expect((await listDeliveryMessages(delivery.id))[0].status).toBe('succeeded');
      } finally {
        rmSync(root, { recursive: true, force: true });
      }
    }));
  ...
  it.each(['concurrent push', 'concurrent rewind', 'revoked access'] as const)(
    ...
    (scenario) =>
      rollbackAfter(async () => {
        ...
        try {
          const local = localRuntime(root, true, async () => {
            if (scenario === 'concurrent rewind') {
              local.git('update-ref', 'refs/heads/feature', `${local.originalHead}^`);
            } else if (scenario === 'revoked access') {
              await execute(
          ...
          else if (scenario === 'concurrent rewind')
            expect(local.git('rev-parse', 'feature')).toBe(
              local.git('rev-parse', `${local.originalHead}^`),
            );
  ...
  it('retains an accepted turn for recovery when queue delivery fails', () =>
    rollbackAfter(async () => {
      ...
      const accepted = await Effect.runPromise(
        Effect.gen(function* () {
          return yield* (yield* DeliveryService).createChatTurn(tenant.user, delivery.id, 'Fix it');
        }).pipe(
          Effect.provide(
