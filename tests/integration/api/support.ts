...
import type { CurrentUserIdentity } from '../../../src/api/contract/auth.ts';
import { ApiDependencies, type ApiRuntimeDependencies } from '../../../src/api/server/context.ts';
import { execute, queryOne, withTransaction } from '../../../src/data/postgres.ts';
import { createSkillsShClient } from '../../../src/integrations/skills-sh/client.ts';
...
export function recordingApiDependencies(messages: RunFactoryMessage[]) {
  return Layer.succeed(ApiDependencies, apiDependencies(messages));
}

export function apiDependencies(
  messages: RunFactoryMessage[],
  authenticate: ApiRuntimeDependencies['authenticate'] = async () => null,
