...
- `src/routes/api.ts` — session-cookie-authed JSON API under `/api/*` that the SPA consumes (dashboard, reviews, factory/plans/cockpit, agents + MCP connections, per-repo settings). Response shapes live in `src/shared/api-types.ts`, type-checked in both the worker and client programs.
