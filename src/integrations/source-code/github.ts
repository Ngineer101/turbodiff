...
// GitHub source-code viewer/editor: all content is read and written
...
const TREE_ENTRY_TYPES = new Set<RepositoryTreeEntry['type']>([
  'dir',
  'file',
  'symlink',
  'submodule',
...
function entryType(type: string): RepositoryTreeEntry['type'] {
  // SAFETY: the membership check proves `type` is one of the union's values.
  return TREE_ENTRY_TYPES.has(type as RepositoryTreeEntry['type'])
    ? (type as RepositoryTreeEntry['type'])
    : 'file';
}

// The tree panel's display order — dirs first, each group name-sorted.
// Shared with the Artifacts source-code adapter.
export function sortTreeEntries(entries: RepositoryTreeEntry[]): void {
  entries.sort(
    (a, b) =>
...
export async function readTree(
  ...
  const entries: RepositoryTreeEntry[] = data.map((entry) => {
...
async function readPreviewFallback(
  ...
  ref: string,
  path: string,
): Promise<RepositoryFileResult | null> {
...
export async function readFile(
  ...
  const file: RepositoryFileResult = {
    path,
    ref,
    sha: data.sha,
    size: data.size,
...
export async function saveFile(
  ...
  let pr: SaveRepositoryFileResult['pr'] = null;
  if (input.mode === 'pr') {
