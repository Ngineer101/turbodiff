...
import { openJson, openToken, sealJson } from '../security/crypto.ts';
import {
...
function oauthRedirectUri(connectionId: number): string {
  return `${env.PUBLIC_BASE_URL}/protocol/connections/${connectionId}/oauth/callback`;
}
...
export async function startOAuthConnect(conn: ConnectionRow): Promise<OAuthConnectStart> {
  ...
  // A route migration changes the registered callback. Dynamic clients must
  // be registered again rather than silently reusing an id whose provider
  // will reject the new redirect_uri.
  if (!clientId || cache.redirectUri !== redirectUri) {
