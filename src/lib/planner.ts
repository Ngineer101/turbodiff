...
import { createFeature, getPlan, listReposForPlan, updatePlan, type PlanRow, type RepositoryRow } from './db.ts';
...
function reposList(dirs: { repo: RepositoryRow; dir: string }[]): string {
	return dirs.map(({ repo, dir }) => `- ${repo.owner}/${repo.name} — checked out at ${dir}`).join('\n');
}
	...
	try {
		...
		if (sandbox) {
			await Promise.all(
				dirs.map(({ repo, dir }) =>
					sandbox!
						.exec(`git -C ${dir} remote set-url origin "https://github.com/${repo.owner}/${repo.name}.git"`)
						.catch(() => {}),
				),
			);
		}
