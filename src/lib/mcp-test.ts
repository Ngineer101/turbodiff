...
function parseRpcBody(text: string): { result?: unknown; error?: { message?: string } } | null {
	const candidates = text.startsWith('data:') || text.includes('\ndata:')
		? text
				.split('\n')
...
export async function testMcpEndpoint(url: string, token?: string): Promise<McpTestResult> {
	...
	try {
		...
		return { ok: false, detail: `could not reach server: ${err instanceof Error ? err.message : String(err)}` };
	}
	if (!initRes.ok) {
		return {
			ok: false,
	...
	if (!init || init.error) {
		return { ok: false, detail: `initialize failed: ${init?.error?.message ?? 'unparseable response'}` };
	}
