...
import { computeRiskTierFromFiles } from '../reviews/policy.ts';
...
describe('maybeAutoMergeCr gates', () => {
  it('never merges when the repo has not opted in or the CR is not clean', async () => {
    ...
    // The full-green path reaches the sandbox engine and is covered by deployment smoke tests.
  ...
  it('escalates sensitive paths regardless of size', () => {
    expect(
      computeRiskTierFromFiles([
        { filename: 'src/application/auth/session.ts', additions: 1, deletions: 0 },
      ]),
    ).toBe('full');
  });
