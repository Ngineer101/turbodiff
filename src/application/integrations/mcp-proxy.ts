...
import { resolveIntegrationAuth } from './credentials.ts';
...
export async function proxyMcpRequest(integrationId: number, request: Request): Promise<Response> {
  ...
  let auth: Awaited<ReturnType<typeof resolveIntegrationAuth>>;
  try {
    auth = await resolveIntegrationAuth(integration);
  } catch (error) {
