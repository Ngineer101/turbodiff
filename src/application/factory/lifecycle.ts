...
import { publishReviewReadinessCheckForStage } from '../reviews/readiness-check.ts';
import type {
  LifecycleDecision,
  LifecycleEventKind,
  LifecycleStage,
...
} from '../reviews/change-review.ts';
import { mergePullRequest } from '../deliveries/auto-merge.ts';
import { enqueueFactoryMessage } from './queue.ts';
import { checkMergeability } from '../deliveries/merge-conflicts.ts';
import { computePushDelta, computeRiskTier } from '../reviews/policy.ts';
...
async function resumeStoppedImplementation(
  ...
  if (previous.status === 'running') {
    // A recent claim is still starting; the idempotent retry can return it.
...
const reviewStageArtifactsSchema = z.object({
  artifacts: z.array(z.object({ agent: z.string().min(1), key: z.string().min(1) }).strict()),
});
...
export async function runLifecycleStage(
  ...
  if (stageRun.stage === 'repair') {
    try {
      ...
      if (!findings)
        throw new Error('repair input artifact is unavailable or contains no findings');
