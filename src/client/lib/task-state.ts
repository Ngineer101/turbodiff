...
export function taskColumn(p: ApiPlan): 'in_progress' | 'done' {
	return p.repos.length > 0 && p.repos.every((r) => r.feature_status === 'merged') ? 'done' : 'in_progress';
}
...
export function taskState(p: ApiPlan): { label: string; tone: TaskTone; hint: string } {
	switch (p.status) {
		...
		case 'approved': {
			...
			if (merged > 0) {
				return { label: `${merged}/${total} merged`, tone: 'on', hint: 'some repos are still in flight' };
			}
			if (open > 0) {
				return {
