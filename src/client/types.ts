...
// One agent run and its lazily fetched execution log.
...
export interface ApiAgentDetail {
  agent: ApiAgentSummary & { instructions: string };
...
// Roles are scoped per organization, so the members page fetches the role for
// the organization it is showing.
...
// One repository attachment of an MCP integration.
...
// An integration belongs to one organization and only attaches to that
