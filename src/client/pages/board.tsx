				...
				return (
					<button
						...
						className={cn(
							...
							selected ? 'border-accent/40 bg-accent/15 text-accent-bright' : 'border-line-2 text-mute hover:text-ink',
						)}
					>
						{r.owner}/{r.name}
					</button>
	...
	return (
		<Card className="animate-rise p-3 transition-colors hover:border-accent/30">
			...
			<div className="mt-1.5 flex flex-wrap items-center gap-x-2 gap-y-1">
				...
				{task.repos
					...
					.map((r) => (
						<Pill
							key={r.repository_id}
							tone={r.verification!.status === 'passed' ? 'on' : r.verification!.status === 'running' ? 'running' : 'red'}
						>
			...
			<div className="mt-2 flex flex-wrap items-center justify-between gap-x-2 text-xs text-mute">
				<span className="min-w-0 truncate">{task.repos.map((r) => `${r.owner}/${r.name}`).join(', ')}</span>
