					...
					<li key={f.filename}>
						<button
							...
							className={cn(
								...
								active ? 'bg-raised text-accent-bright' : 'text-ink-dim hover:bg-raised/60 hover:text-ink',
							)}
							style={{ paddingLeft: `${depth * 0.75 + 0.375}rem` }}
							title={`${f.filename} (${f.status})`}
						>
							<span
								className={cn('size-1.5 shrink-0 rounded-full', FILE_STATUS_DOT[f.status] ?? 'bg-mute')}
								aria-hidden
							/>
